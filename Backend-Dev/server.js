...
const server = http.createServer((req, res) => {
  ...
  if (parsedUrl.pathname === "/product") {
    ...
    if (!name || !price || !discount) {
      ...
      return res.end("<h2>Error: Missing query parameters</h2>");
    }

    const finalPrice = price - (price * discount) / 100;

    const logData = `Product: ${name}, Price: ${price}, Discount: ${discount}%, Final Price: ${finalPrice}\n`;
    ...
    const html = `
      <html>
        <head><title>Product Result</title></head>
